## Current
Pre-E2E stabilization sweep: stop chasing new demo flows until the remaining yellow surfaces are closed enough to trust another full run. Immediate focus is startup/test signal reform, disconnect/degraded-mode behavior, and high-noise diagnostics gaps revealed by the latest live session.
...
Live deploy-routing truth audit: re-check the `rule_deploy_already_deployed` short-circuit against current world truth so deploy feedback is only blocked when the world model really supports that conclusion.
