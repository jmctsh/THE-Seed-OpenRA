## Current
Yellow-zone audit before next E2E: read `adjutant/`, `kernel/`, `world_model/`, `dashboard/web-console-v2`, and `tests/` runtime/diagnostics coverage, then report only the remaining P1/P2 items that still reduce E2E trustworthiness or debugging UX, with concrete file-level causes and three minimal next slices.

## Queue
Testing reform / signal quality: layered backend gate is now in place (`contract`, `startup_smoke`, `runtime_invariants`, `mock_integration`, plus manual `live`), so the next step is to widen the trustworthy slices without collapsing back into one broad green count.
...
Test-signal audit follow-up: broader capability/diagnostics E2E coverage still remains, but the startup smoke, layered backend gate, bootstrap smoke, and several stale assertion surfaces are now closed.
