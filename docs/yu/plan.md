## Current
Testing reform / signal quality: layered backend gate, direct-entry startup smoke, E2E classification guardrails, and pytest-backed `__main__` runners are now in place (`contract`, `startup_smoke`, `runtime_invariants`, `mock_integration`, plus manual `live`), so the next step is to widen the trustworthy slices without collapsing back into one broad green count.
