...
## [2026-04-06] DONE — Capability 行为三大根因修复

用户 E2E 测试发现 EconomyCapability 行为完全错误：乱造兵、不理解可造规则、看不到 request。

根因分析：
1. `unfulfilled_requests` 从未注入 runtime_facts — Kernel 有 `_unit_requests` 数据但 `set_runtime_state()` 没有传递给 WorldModel，导致 `[待处理请求]` 永远为空
2. `production_queues` 同理缺失 — `[生产队列]` 永远为空，Capability 不知道正在造什么
3. `CAPABILITY_SYSTEM_PROMPT` 太主动 — "电力不足→建电厂、矿车不足→造矿车、科技升级" 让 LLM 在没有请求时也疯狂造东西

修复：
- `world_model/core.py`: `set_runtime_state()` 新增 `unfulfilled_requests` 参数 + `_unfulfilled_requests` 字段；`compute_runtime_facts()` 输出 `unfulfilled_requests` 和 `production_queues`
- `kernel/core.py`: `_sync_world_model()` 序列化 pending/partial `_unit_requests` 传入 WorldModel
- `task_agent/agent.py`: 重写 `CAPABILITY_SYSTEM_PROMPT` — 从"主动经济规划总管"改为"被动按需调度器"，只在有请求/玩家指令/低电力时行动，否则输出 wait

测试：81/82 通过（1 个 pre-existing NLU"爆兵"拦截问题）

## [2026-04-06] DONE — Direct job task 不自动关闭 bug 修复

根因：Expert job succeed 后 emit `SignalKind.TASK_COMPLETE`，`route_signal()` 把信号推给 `runtime.agent`，但 direct-managed task 的 agent 从未启动（skip_agent=True），信号无人消费 → task 永远 RUNNING。

修复：`kernel/core.py` `route_signal()` — 当 `TASK_COMPLETE` 信号到达 direct-managed task 时，直接调用 `complete_task()` 而不是推给 agent。
